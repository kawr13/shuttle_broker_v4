#!/usr/bin/env python3
"""
Тестирование упрощенной версии шлюза для шаттлов (только CRLF)
"""
import asyncio
import argparse
import sys
...
from shuttle_module.shuttle_manager_simple import get_shuttle_manager
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('shuttle_test_simple.log')
    ]
)
logger = logging.getLogger("test_shuttle_simple")

async def test_direct_command(ip, port, command):
    """Тестирует прямую отправку команды шаттлу с терминатором CRLF"""
    try:
        # Создаем временный клиент
        shuttle = ShuttleClientSimple("test_shuttle")
...
async def main():
    """Основная функция"""
    parser = argparse.ArgumentParser(description="Тестирование упрощенного шлюза для шаттлов (только CRLF)")
    ...
    if args.listen:
        ...
        await test_direct_command(args.ip, args.port, args.command)
    elif args.shuttle:
