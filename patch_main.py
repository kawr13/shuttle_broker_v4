#!/usr/bin/env python3
"""
Патч для main.py, добавляющий применение патча ShuttleManager при запуске
"""
import asyncio
import argparse
import os
...
# Применяем патч к ShuttleManager
...
async def main(config_file: Optional[str] = None):
    ...
    logger.info("Запуск шлюза WMS-Шаттл (Версия 3.4)...")
    logger.info("Применен патч для поддержки автоматически обнаруженных шаттлов")
...
async def shutdown():
    ...
    logger = get_logger()
    logger.info("Остановка шлюза WMS-Шаттл (Версия 3.4)...")
