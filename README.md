...
3. Улучшена обработка соединений:
...
## Постоянные TCP-соединения (версия 3.5)
...
2. **Менеджер соединений**:
   ...
   - Синхронизация доступа через блокировки
...
## Автоматическое обнаружение шаттлов (версия 3.4)

В версии 3.4 добавлен модуль автоматического обнаружения новых шаттлов в сети:

### Основные возможности:

1. **Автоматическое сканирование сети**:
   - Периодическое сканирование локальных сетей на предмет новых устройств
   - Поддержка множественных диапазонов IP-адресов
   - Настраиваемый интервал сканирования (по умолчанию 30 секунд)

2. **Идентификация шаттлов**:
   - Автоматическое определение устройств, отвечающих на команды шаттлов
   - Сбор информации о статусе, уровне батареи и местоположении
   - Присвоение уникальных имен новым шаттлам

3. **Автоматическая регистрация**:
   - Автоматическое добавление обнаруженных шаттлов в систему
   - Создание конфигурации и инициализация клиентов
   - Интеграция с существующими компонентами системы

4. **Гибкое местоположение**:
   - Автоматический запрос текущего местоположения новых шаттлов
   - Отсутствие жесткой привязки к конкретным ячейкам
   - Динамическое обновление информации о местоположении

### API для работы с обнаруженными шаттлами:

```bash
# Просмотр обнаруженных шаттлов через API
curl http://localhost:8000/discovered

# Просмотр статуса всех шаттлов
...
```bash
# Показать обнаруженные шаттлы
python cli.py discovered

# Сканировать сеть вручную
python cli.py scan

# Сканировать конкретный диапазон
python cli.py scan --ip-range 192.168.1
```
...
Модуль автоматически запускается вместе с основным приложением и работает в фоновом режиме, обеспечивая непрерывное обнаружение новых шаттлов без необходимости ручной настройки.
## Автоматическое обнаружение и регистрация шаттлов

Система автоматически:

1. **Сканирует сеть** каждые 30 секунд для поиска новых устройств
2. **Идентифицирует шаттлы** по их способности отвечать на стандартные команды
3. **Собирает информацию** о статусе, батарее и местоположении
4. **Автоматически регистрирует** новые шаттлы в системе
5. **Присваивает имена** в формате `shuttle_IP_ADDRESS`
6. **Запрашивает местоположение** для гибкого позиционирования

Это обеспечивает:
- Отсутствие необходимости в ручной настройке новых шаттлов
- Автоматическое восстановление после сбоев сети
- Гибкость в размещении шаттлов без привязки к конкретным ячейкам
- Масштабируемость системы при добавлении новых устройств
