...
В версии 3.4 добавлена функция автоматического обнаружения и регистрации шаттлов:

1. При подключении нового шаттла с неизвестным IP-адресом:
   - Шаттлу присваивается временный ID в формате `temp_shuttle_10_181_80_145`
   - Шлюз отправляет запросы STATUS и MRCD для получения информации о шаттле

2. Когда шаттл отправляет сообщение STATUS или LOCATION:
   - Шаттл автоматически регистрируется в конфигурации с постоянным ID
   - IP-адрес шаттла сохраняется в конфигурации
   - Информация о местоположении шаттла (ячейке) также сохраняется

3. После регистрации:
   - Конфигурация перезагружается без перезапуска шлюза
   - Шаттл становится доступен для получения команд от WMS
   - Временный ID заменяется на постоянный

Это позволяет шлюзу автоматически обнаруживать и подключать новые шаттлы без необходимости ручного редактирования конфигурации.