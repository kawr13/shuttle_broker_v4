...
from shuttle_module.shuttle_client_simple import ShuttleClientSimple
from shuttle_module.shuttle_listener_v2 import get_shuttle_listener

logger = logging.getLogger(__name__)

class ShuttleManagerSimple:
    """Упрощенный менеджер для управления шаттлами"""
    
    def __init__(self):
        self.shuttles: Dict[str, ShuttleClientSimple] = {}
        self.running = False
    ...
    async def add_shuttle(self, shuttle_id: str) -> bool:
        ...
        try:
            ...
            shuttle = ShuttleClientSimple(shuttle_id)
    ...
    async def send_command(self, shuttle_id: str, command: str, params: str = None, max_retries: int = 3) -> bool:
        """
        ...
        for attempt in range(max_retries):
            success = await shuttle.send_command(command, params)
            if success:
                return True
    ...
    def get_shuttle(self, shuttle_id: str) -> Optional[ShuttleClientSimple]:
        """Возвращает клиент шаттла по ID"""
        return self.shuttles.get(shuttle_id)
    
    def get_all_shuttles(self) -> Dict[str, ShuttleClientSimple]:
    ...
    async def _check_shuttles_health(self):
        ...
        for shuttle_id, shuttle in self.shuttles.items():
            try:
                ...
                if not shuttle.last_seen or (time.time() - shuttle.last_seen) > 30:
